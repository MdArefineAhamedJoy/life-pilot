...
try { process.loadEnvFile(path.join(backend, ".env")); } catch { /* Environment variables may be supplied directly. */ }
const dbUrl = process.env.DATABASE_URL ? new URL(process.env.DATABASE_URL) : new URL("postgres://localhost");
...
const admin = new Pool({ connectionString: dbUrl.toString(), connectionTimeoutMillis: 5000,
...
async function start(entry, args, cwd, env, label) {
  ...
  logs.push(log);
  const child = spawn(process.execPath, [entry, ...args], { cwd, env: { ...process.env, ...env }, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
...
async function waitFor(url, child) {
  for (let attempt = 0; attempt < 120; attempt++) {
    if (child.exitCode !== null) throw new Error("Test server exited; inspect .tmp integration logs.");
    try { if ((await fetch(url, { signal: AbortSignal.timeout(3000) })).ok) return; } catch { /* Wait for server startup. */ }
    await sleep(500);
  }
...
async function request(context, method, endpoint, data, status) {
  ...
  check(status ? response.status() === status : response.ok(), method + " " + endpoint + ": " + response.status() + " " + await response.text());
...
try {
  ...
  const journal = JSON.parse(await readFile(path.join(backend, "drizzle/meta/_journal.json"), "utf8"));
  for (const migration of journal.entries) {
    const sql = (await readFile(path.join(backend, "drizzle", migration.tag + ".sql"), "utf8")).replaceAll('"public".', '"' + schema + '".');
    await database.query(sql);
  }
  console.log("Isolated database schema prepared.");
  const apiServer = await start(path.join(backend, "dist/main.js"), [], backend,
    { DATABASE_URL: dbUrl.toString(), API_PORT: apiPort, CORS_ORIGIN: web }, "integration-api");
  ...
  const webServer = await start(path.join(standalone, "server.js"), [], standalone,
    { API_BASE_URL: api, PORT: webPort, HOSTNAME: "localhost", NODE_ENV: "production" }, "integration-web");
  ...
  const routes = ["/", "/dashboard", "/budget", "/expenses", "/categories", "/tasks", "/routine", "/timer", "/notes", "/ai", "/calendar",
    "/reports", "/shopping", "/health", "/family", "/goals", "/meal-planner", "/reminder", "/settings", "/profile", "/scan-slip", "/receipt-scanner"];
  for (const route of routes) {
    const response = await guest.request.get(route, { maxRedirects: 0 });
    check(response.status() === 307 && response.headers().location.includes("/login"), "Guest route blocked: " + route);
  }
  const guarded = ["auth/me", "account/profile", "health/db", "life-os/state", "life-os/categories", "life-os/expenses",
    "life-os/tasks", "life-os/timer-sessions", "life-os/notes", "life-os/settings"];
  ...
  const userA = { name: "Integration A", email: "a-" + schema + "@example.test", phone: "0123456789", password: "Integration-test-password-73!" };
  const userB = { ...userA, name: "Integration B", email: "b-" + schema + "@example.test" };
  const page = await a.newPage();
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.goto("/register");
  await page.evaluate(() => localStorage.setItem("life-pilot-state-v2", JSON.stringify({ notes: [{ id: "stale-cache", title: "Stale private note" }] })));
  ...
  check(cookies.some((cookie) => cookie.name === "life-pilot-session" && cookie.httpOnly && cookie.sameSite === "Lax"), "HttpOnly session cookie");
  check(await page.evaluate(() => localStorage.getItem("life-pilot-auth")) === null, "No token in localStorage");
  const signupB = await request(b, "POST", "auth/register", userB);
  check(!("token" in signupB), "BFF does not expose bearer tokens");
  const csrf = await a.request.post(web + "/api/life-os/notes", { headers: { origin: "https://untrusted.example" }, data: { title: "Blocked", body: "" } });
  check(csrf.status() === 403, "Cross-origin mutations are blocked");
  check((await a.request.get("/login", { maxRedirects: 0 })).headers().location.endsWith("/dashboard"), "Authenticated login page redirects");
  const empty = await request(a, "GET", "life-os/state");
  for (const key of ["categories", "expenses", "tasks", "timerSessions", "notes"]) check(empty[key].length === 0, "New account starts empty: " + key);
  ...
  check(expense.itemName === "Integration rice" && expense.amount === 120, "Expense form persists through CRUD");
  await page.getByRole("button", { name: "Add Expense", exact: true }).click();
  dialog = page.getByRole("dialog");
  await dialog.locator('[name="itemName"]').fill("Keep this draft");
  await dialog.locator('[name="amount"]').fill("30");
  await page.route("**/api/life-os/expenses", (route) => route.request().method() === "POST"
    ? route.fulfill({ status: 503, contentType: "application/json", body: JSON.stringify({ message: "Test save failure" }) }) : route.continue());
  await dialog.getByRole("button", { name: "Save expense", exact: true }).click();
  ...
  check(writes.some((url) => url.endsWith("/life-os/categories")) && writes.some((url) => url.endsWith("/life-os/expenses")) &&
    writes.some((url) => url.endsWith("/life-os/tasks")) && !writes.some((url) => url.endsWith("/life-os/state")), "Forms use focused CRUD, not snapshot autosave");

  await request(a, "PUT", "life-os/categories/" + category.id, { ...category, name: "Renamed groceries", monthlyLimit: 3000 });
  check((await request(a, "GET", "life-os/expenses"))[0].category === "Renamed groceries", "Category rename propagates");
  await request(a, "POST", "life-os/expenses/bulk", { rows: [{ itemName: "Bulk item", category: "Renamed groceries", amount: 15 }], date: expense.date });
  await request(a, "PATCH", "life-os/tasks/" + task.id, { title: "Updated task" });
  const completed = await request(a, "PATCH", "life-os/tasks/" + task.id + "/status", { status: "completed" });
  check(Boolean(completed.completedAt), "Completion timestamp set");
  const pending = await request(a, "PATCH", "life-os/tasks/" + task.id + "/status", { status: "pending" });
  ...
  await request(a, "POST", "life-os/timer-sessions", { taskId: task.id, title: "Integration focus", category: "Work", mode: "focus", durationSeconds: 60 });
  const note = await request(a, "POST", "life-os/notes", { title: "Integration note", body: "Saved content", tags: ["shopping", "health"] });
  await request(a, "PUT", "life-os/notes/" + note.id, { title: "Updated note", body: "Changed content", tags: ["shopping", "health"] });
  await request(a, "PATCH", "life-os/settings", { currency: "USD", quietHoursStart: "21:00" });
  await request(a, "POST", "account/profile", { name: "Updated profile", phone: "0987654321", bio: "Integration profile" });
  check((await request(a, "GET", "life-os/settings")).profileName === "Updated profile", "Profile stays consistent with settings");
  ...
  await request(a, "POST", "life-os/expenses", { itemName: "Invalid", category: "Invalid", amount: -10, date: expense.date }, 400);
  ...
  check((await request(a, "GET", "life-os/state")).expenses.length === 2, "Backup replacement persists");

  for (const key of ["categories", "expenses", "tasks", "timer-sessions", "notes"]) check((await request(b, "GET", "life-os/" + key)).length === 0, "User B isolated: " + key);
  await request(b, "PATCH", "life-os/tasks/" + task.id, { title: "Cross-account edit" }, 404);
  await request(b, "PUT", "life-os/notes/" + note.id, { title: "Cross-account edit", body: "" }, 404);
  await request(b, "DELETE", "life-os/expenses/" + expense.id);
  check((await request(a, "GET", "life-os/expenses")).length === 2, "Cross-account delete cannot remove A's data");
  ...
  await page.screenshot({ path: path.join(frontend, ".tmp/integration-desktop.png"), fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("/dashboard");
  await expect(page.getByRole("heading", { name: "Welcome back", exact: true })).toBeVisible();
  check((await page.locator("main").innerText()).includes("$"), "Saved currency is reflected in dashboard amounts");
  check(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), "Mobile dashboard has no horizontal overflow");
  await page.screenshot({ path: path.join(frontend, ".tmp/integration-mobile.png"), fullPage: true });
  ...
  check((await request(a, "GET", "life-os/state")).timerSessions.length === 0, "Reset clears actual data");
  check((await request(b, "GET", "life-os/notes"))[0].title === "B private note", "Reset does not affect another account");
  ...
  console.log("PASS: " + checks + " assertions plus UI form, route, failure, mobile, login and logout checks.");
} finally {
  await browser?.close();
  for (const child of children.reverse()) {
    if (child.exitCode === null) { child.kill(); await Promise.race([once(child, "exit"), sleep(3000)]); }
  }
  for (const log of logs) log.end();
  await database?.end();
  if (schemaCreated && /^life_pilot_e2e_[a-f0-9]{16}$/.test(schema)) await admin.query('DROP SCHEMA "' + schema + '" CASCADE');
  await admin.end();
}
