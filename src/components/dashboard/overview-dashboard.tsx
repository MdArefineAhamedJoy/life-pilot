...
function EmptyState({ children }: { children: string }) {
  return (
    <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm font-medium text-slate-500">
      {children}
    </p>
  );
}
        ...
        <div className="grid h-64 grid-cols-7 items-end gap-3 border-b border-slate-200 px-2 pt-8">
          {points.map((point) => (
            <div className="flex h-full min-w-0 flex-col justify-end gap-2" key={point.day}>
              <span
                className="truncate text-center font-mono text-xs font-semibold text-slate-600"
                title={formatCurrency(point.value)}
              >
                {point.value ? formatCurrency(point.value) : "—"}
              </span>
              <span
                className="min-h-1 rounded-t bg-emerald-500"
                style={{
                  height: `${Math.max((point.value / maxValue) * 100, point.value ? 3 : 1)}%`,
                }}
              />
              <span className="pb-3 text-center text-xs font-semibold text-slate-500">
                {point.day}
              </span>
            </div>
          ))}
...
function CategoryDistribution({
  ...
  return (
    <SharedCard>
              ...
              <div key={row.id}>
                <div className="mb-2 flex items-center justify-between gap-4 text-sm font-semibold">
                  ...
                  <span className="shrink-0 font-mono text-slate-600">
                    {formatCurrency(row.spent)} · {percent}%
                  </span>
                </div>
      ...
      <Link
        className="mt-8 flex min-h-12 items-center justify-center gap-2 border border-slate-200 text-sm font-semibold text-slate-800 hover:border-emerald-500/40 hover:text-emerald-600"
        href="/budget"
      >
  ...
  return (
    <SharedCard>
      ...
      {sortedTasks.length === 0 ? (
        <EmptyState>Add a task to build your routine.</EmptyState>
      ) : (
        <div className="space-y-3">
          {sortedTasks.map((task) => (
            <div
              className="grid grid-cols-[74px_minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3"
              key={task.id}
            >
...
function RecentExpenses({ expenses }: { expenses: Expense[] }) {
  ...
  return (
    <SharedCard>
      ...
      {rows.length === 0 ? (
        <EmptyState>Your saved expenses will appear here.</EmptyState>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm">
            <thead>
              <tr className="border-y border-slate-200 bg-slate-50 text-left font-semibold text-slate-500">
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((expense) => (
                <tr key={expense.id}>
                  <td className="px-3 py-4 font-semibold text-slate-950">{expense.itemName}</td>
                  <td className="px-3 py-4 text-slate-700">{expense.category}</td>
                  <td className="px-3 py-4 text-slate-600">{expense.date}</td>
                  <td className="px-3 py-4 text-right font-mono font-semibold text-slate-950">
...
function UpcomingReminders({ tasks }: { tasks: RoutineTask[] }) {
  const reminders = tasks
    .filter((task) => task.alertEnabled && task.reminderAt)
    .sort((a, b) => (a.reminderAt ?? "").localeCompare(b.reminderAt ?? ""));
  return (
    <SharedCard>
      <SharedCardHeader title="Upcoming Reminders" />
      {reminders.length === 0 ? (
        <EmptyState>Enable a task reminder to see it here.</EmptyState>
      ) : (
        <div className="space-y-3">
          {reminders.map((task) => (
            <div
...
function TaskSummary({ tasks }: { tasks: RoutineTask[] }) {
  ...
  const missed = tasks.filter(
    (task) => task.status === "missed" || task.status === "delayed"
  ).length;
  ...
  return (
    <SharedCard>
      <SharedCardHeader title="Today's Tasks" />
      {tasks.length === 0 ? (
        <EmptyState>Add a task to begin planning your day.</EmptyState>
      ) : (
        <>
          <div className="flex flex-wrap gap-2">
            {stats.map((stat) => (
              ...
              >
                <span
                  className={cn("flex size-5 items-center justify-center rounded-full", stat.tone)}
                >
            ...
            <div className="mb-3 flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-xs font-medium text-slate-500">
                  {tasks.length} tracked task{tasks.length === 1 ? "" : "s"}
                </p>
              </div>
              <span className="font-mono text-2xl font-semibold text-emerald-600">{progress}%</span>
            </div>
          ...
          <div className="mt-4 space-y-2">
            {nextTasks.map((task) => (
              <div
                className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3"
                key={task.id}
              >
    ...
    <section className="min-w-0 space-y-6 overflow-x-hidden" id="dashboard">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm font-medium text-slate-500">{dateLabel}</p>
        </div>
        <Link
          className="text-sm font-semibold text-emerald-600 hover:text-emerald-700"
          href="/settings"
      ...
      <div className="grid min-w-0 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          progress={
            totalBudget > 0 ? Math.min(Math.round((todaySpent / totalBudget) * 100), 100) : 0
          }
          tone="red"
          value={formatCurrency(todaySpent)}
        />
        <StatCard
          detail={remainingBudget < 0 ? "over budget" : "available to spend"}
          icon={WalletCards}
          label="Remaining Budget"
          progress={totalBudget > 0 ? Math.max(100 - budgetProgress, 0) : 0}
          tone="blue"
          value={formatCurrency(Math.max(remainingBudget, 0))}
        />
        <StatCard
          detail={`${budgetProgress}% used`}
          icon={PiggyBank}
          label="Budget Usage"
          progress={budgetProgress}
          tone="amber"
