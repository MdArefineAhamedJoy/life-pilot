...
export function RoutineAddPointModal({
  nextOrder,
  onAddPoint,
  onOpenChange,
  open,
}: RoutineAddPointModalProps) {
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="flex h-[82vh] !w-[min(92vw,820px)] max-w-none grid-rows-none flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="shrink-0 border-b border-slate-200 px-4 py-3">
          ...
          <DialogDescription>
            Write one notebook line with time, duration, repeat, and alert preference.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="grid min-w-0 gap-4 p-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <FieldShell label="Point title">
                  <TextInput
                    name="title"
                    placeholder="Fajr, exercise, work block, bajar list"
                    required
                  />
                </FieldShell>
              </div>
              ...
              <div className="md:col-span-2">
                <FieldShell label="Note">
                  <TextArea
                    className="min-h-24"
                    name="note"
                    placeholder="Optional details for this routine point"
                  />
                </FieldShell>
              </div>
            </div>
