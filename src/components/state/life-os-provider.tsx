...
type LifeOsContextValue = LifeOsState & {
  ...
  addTask: (
    task: Omit<RoutineTask, "id" | "status" | "repeatRule"> &
  ...
  updateNote: (
    id: string,
    note: Pick<LifeNote, "title" | "body"> & { tags?: string[] }
  ) => Promise<boolean>;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled)
          setError(cause instanceof Error ? cause.message : "Your data could not be loaded.");
      }
    }
    void load();
    return () => {
    ...
    (operation: (state: LifeOsState) => Promise<LifeOsState>) => {
      ...
      const result = queue.current.then(async () => {
        ...
        try {
          ...
          if (active.current)
            setError(
              cause instanceof Error ? cause.message : "Changes were not saved. Please try again."
            );
          return false;
        } finally {
  ...
  const value = useMemo<LifeOsContextValue>(
    () => ({
      ...
      addBudgetCategory: (payload) =>
        mutate(async (s) => ({
          ...s,
          categories: [...s.categories, await categoriesService.create(payload)],
        })),
      updateBudgetCategory: (id, payload) =>
        mutate(async (s) => {
          ...
          return {
            ...s,
            categories: s.categories.map((item) => (item.id === id ? category : item)),
            expenses: s.expenses.map((item) =>
              previous && item.category === previous.name
      ...
      addExpense: (payload) =>
        mutate(async (s) => ({
          ...s,
          expenses: [await expensesService.create(payload), ...s.expenses],
        })),
      addExpensesFromRows: (rows, date = localDateKey()) =>
        mutate(async (s) => ({
          ...s,
          expenses: [...(await expensesService.createBulk(rows, date)), ...s.expenses],
        })),
      deleteExpense: (id) =>
        mutate(async (s) => {
        ...
        mutate(async (s) => ({
          ...
          tasks: [
            ...
            await tasksService.create({
              ...payload,
              order: payload.order ?? s.tasks.length + 1,
              status: payload.status ?? "pending",
              repeatRule: payload.repeatRule ?? "daily",
            }),
          ],
        })),
      ...
      reorderTasks: (ids) =>
        mutate(async (s) => ({ ...s, tasks: await tasksService.reorder(ids) })),
      addTimerSession: (payload) =>
        mutate(async (s) => ({
          ...s,
          timerSessions: [await timerSessionsService.create(payload), ...s.timerSessions],
        })),
      addNote: (payload) =>
        mutate(async (s) => ({ ...s, notes: [await notesService.create(payload), ...s.notes] })),
      ...
      updateSettings: (payload) =>
        mutate(async (s) => ({ ...s, settings: await settingsService.update(payload) })),
      restoreData: (payload) =>
        mutate(async () => lifeOsStateService.replace(validateBackup(payload))),
  ...
  return (
    <LifeOsContext.Provider value={value}>
      {error && (
        <div
          role="alert"
          className="fixed inset-x-4 top-3 z-[100] rounded-lg border border-red-300 bg-red-50 p-4 text-red-800 shadow-lg"
        >
          {error}{" "}
      ...
      {pending > 0 && (
        <div
          role="status"
          className="fixed right-4 top-4 z-[100] rounded-lg bg-slate-900 px-4 py-2 text-sm text-white"
        >
          Saving…
        </div>
      )}
      <WorkspaceSavingContext.Provider value={pending > 0}>
        <fieldset disabled={pending > 0} className="contents">
          {children}
        </fieldset>
